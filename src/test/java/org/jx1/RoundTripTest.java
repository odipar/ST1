...
final class RoundTripTest {
    ...
    private static byte[] compress(byte[] input) {
        return Compressor.compress(Optimizer.optimize(input, 0, Zx1.MAX_OFFSET_ZX1), input, 0, false).output();
    }
    ...
    void quickModeRoundTrips() {
        ...
        byte[] output = Compressor.compress(
                Optimizer.optimize(input, 0, Zx1.MAX_OFFSET_ZX7), input, 0, false).output();
